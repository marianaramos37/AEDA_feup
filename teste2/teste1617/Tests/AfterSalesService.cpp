...
bool sortArt(const Article * left, const Article * right){
    return *left < *right;
}
/**
...
void AfterSalesService::sortArticles() {
    if(!presentsTable.empty()){
        if(presentsTable.size()>10)
            ...
            sort(presentsTable.begin(), presentsTable.end(),sortArt);
    }
}
...
void AfterSalesService::enqueueArticles() {
    ...
    vector<Article*>::iterator it;
    for(it= presentsTable.begin(); it != presentsTable.end(); it++){
        if(toWrap.size() < toWrapQueueSize){
            toWrap.push(*it);
            presentsTable.erase(it);
            it--;
        }
...
Article* AfterSalesService::wrapNext() {
    ...
    if(!toWrap.empty()){
        a = toWrap.front();
        toWrap.pop();
        if(a->getDeliverHome()) toDeliver.push(a);
        else return a;
    }
