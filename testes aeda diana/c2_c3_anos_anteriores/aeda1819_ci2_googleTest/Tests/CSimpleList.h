...
public:
	...
	{
        CNode *this_curr = first,*other_curr=lst.getFirst();

        while(true)
        {
            if(other_curr != 0)
            {
                CNode* newNode = new CNode(other_curr->data(), this_curr->next());
                this_curr->setNext(newNode);
