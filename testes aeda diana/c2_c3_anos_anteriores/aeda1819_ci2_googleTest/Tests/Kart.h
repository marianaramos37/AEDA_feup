...
public:
    ...
    CKart(const CKart & c){
        avariado = c.getAvariado(); nome = c.getNome(); numero = c.getNumero(); cilindrada = c.getCilindrada();}
	string getNome() const{ return nome;}
	float getCilindrada() const{ return cilindrada;}
	int getNumero() const { return numero;}
	bool getAvariado() const{ return avariado; }
	bool operator <(const CKart & ckart) const;
    bool operator ==(const CKart & ckart) const;
};
