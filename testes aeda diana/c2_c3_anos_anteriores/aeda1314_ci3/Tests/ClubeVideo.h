...
public:
	...
	bool operator <(const Filme & f1) const;
};
...
public:
    ...
    void removeFilme(string umFilme){
        stack<Filme> filmesEmprestadosCopy;
        while(!filmesEmprestados.empty()){
            if(!(filmesEmprestados.top().getTitulo() == umFilme))
                filmesEmprestadosCopy.push(filmesEmprestados.top());
            filmesEmprestados.pop();
        }
        filmesEmprestados = filmesEmprestadosCopy;
	}
...
class FilmeInexistente{
    string titulo;
public:
    FilmeInexistente(const string &t){titulo=t;}
    string msg(){
        string s = "Titulo Inexistente: " + titulo;
        return s;
    }
};
