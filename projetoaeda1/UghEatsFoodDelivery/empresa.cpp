...
void Base::addCliente() {
    ...
    if(ln==true){
        cout<<"Devido a pagamentos em atraso nao se pode registar." <<endl;
...
void Base::removeAdmin(Funcionario f) {
    Base b;
    bool a=false;
    if(conselho=="Porto" ||conselho=="Matosinhos"|| conselho=="Vila Nova de Gaia" || conselho=="Maia"||conselho=="Gondomar"){
        b=Base("porto.txt");
    }
    else if(conselho=="Lisboa"|| conselho=="Oeiras" ||conselho=="Amadora" || conselho=="Odivelas"||conselho=="Loures"){
        b=Base("lisboa.txt");
    }
    else if(conselho=="Faro" ||conselho=="Olhao"|| conselho=="Loule" || conselho=="Sao Bras de Alportel"){
        b=Base("faro.txt");
    }
    ofstream file;
    b.funcionarios_readfile();
    for(int i=0; i<b.funcionarios.size();i++){
        if(b.funcionarios[i]->getNif()==f.getNif()){
            if(b.funcionarios[i]->getTipoDeFuncionario()=="Administrador"){
                b.funcionarios.erase(b.funcionarios.begin()+i);
                a=true;
            }else{
                a=false;
            }
    ...
    if(a==true) {
        file.open(b.funcionarios_filename);
        for (int i = 0; i < b.funcionarios.size() - 1; i++) {
            if (funcionarios[i]->getTipoDeFuncionario() == "Administrador") {
                file << "Administrador" << endl;
                file << b.funcionarios[i]->getNome() << endl;
                file << b.funcionarios[i]->getNif() << endl;
                file << to_string(b.funcionarios[i]->getData().getDia()) + "/" +
                        to_string(b.funcionarios[i]->getData().getMes()) + "/" +
                        to_string(b.funcionarios[i]->getData().getAno()) << endl;
                file << to_string(b.funcionarios[i]->getSalario()) << endl;
                file << b.funcionarios[i]->getFuncao() << endl;
                file << ":::::" << endl;
                ...
                file << to_string(b.funcionarios[i]->getData().getDia()) + "/" +
                        ...
                        to_string(b.funcionarios[i]->getData().getAno()) << endl;
                file << to_string(b.funcionarios[i]->getSalario()) << endl;
                file << b.funcionarios[i]->getVeiculoAlocado().getMarca() + ", " +
                        b.funcionarios[i]->getVeiculoAlocado().getTipo() + ", " +
                        b.funcionarios[i]->getVeiculoAlocado().getDataDeAquisicao() << endl;
                if (b.funcionarios[i]->isDisponivel()) {
                    file << "0" << endl;
                } else {
                    file << "1" << endl;
                }
        ...
        if (funcionarios[b.funcionarios.size() - 1]->getTipoDeFuncionario() == "Administrador") {
            file << "Administrador" << endl;
            file << b.funcionarios[b.funcionarios.size() - 1]->getNome() << endl;
            file << b.funcionarios[b.funcionarios.size() - 1]->getNif() << endl;
            file << to_string(b.funcionarios[b.funcionarios.size() - 1]->getData().getDia()) + "/" +
                    to_string(b.funcionarios[b.funcionarios.size() - 1]->getData().getMes()) + "/" +
                    to_string(b.funcionarios[b.funcionarios.size() - 1]->getData().getAno()) << endl;
            file << to_string(b.funcionarios[b.funcionarios.size() - 1]->getSalario()) << endl;
            file << b.funcionarios[b.funcionarios.size() - 1]->getFuncao() << endl;
        } else if (funcionarios[b.funcionarios.size() - 1]->getTipoDeFuncionario() == "Entregador") {
            file << "Entregador" << endl;
            file << b.funcionarios[b.funcionarios.size() - 1]->getNome() << endl;
            file << b.funcionarios[b.funcionarios.size() - 1]->getNif() << endl;
            file << to_string(b.funcionarios[b.funcionarios.size() - 1]->getData().getDia()) + "/" +
                    ...
                    to_string(b.funcionarios[b.funcionarios.size() - 1]->getData().getAno()) << endl;
            file << to_string(b.funcionarios[b.funcionarios.size() - 1]->getSalario()) << endl;
            file << b.funcionarios[b.funcionarios.size() - 1]->getVeiculoAlocado().getMarca() + ", " +
                    b.funcionarios[b.funcionarios.size() - 1]->getVeiculoAlocado().getTipo() + ", " +
                    b.funcionarios[b.funcionarios.size() - 1]->getVeiculoAlocado().getDataDeAquisicao() << endl;
            if (b.funcionarios[b.funcionarios.size() - 1]->isDisponivel()) {
                file << "0" << endl;
            } else {
                file << "1" << endl;
            }
...
void Base::removeEntreg(Funcionario f) {
    ...
    for(int i=0; i<b.funcionarios.size();i++){
        if(b.funcionarios[i]->getNif()==f.getNif()){
            if(b.funcionarios[i]->getTipoDeFuncionario()=="Entregador"){
                b.funcionarios.erase(b.funcionarios.begin()+i);
                a=true;
            }else{
                a=false;
