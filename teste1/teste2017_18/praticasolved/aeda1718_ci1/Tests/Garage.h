...
bool Garage<Vehicle>::addVehicle(Vehicle *vehicle) {
    ...
    if(existe==false && vehicles.size()<capacity) {
...
void Garage<Vehicle>::sortVehicles() {
    for(int j=vehicles.size()-1; j>0; j--)
    {
        ...
        for(int i = 0; i<j; i++) {
            ...
            else if(vehicles[i + 1]->getBrand() == vehicles[i]->getBrand()){
                if(vehicles[i+1]->getPrice()>vehicles[i]->getPrice()){
                    std::swap(vehicles[i],vehicles[i+1]);
                    troca=true;
                }
            }
