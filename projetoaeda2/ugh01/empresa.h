...
public:
    ...
    vector<Funcionario*> getFuncionariosTodos() ; //tabH
    void setFuncionarios(const vector<Funcionario*> &funcionariosv);
    void setFuncionariosTodos(const vector<Funcionario*> &funcionariosv); //tabH
    string getFuncionariosFileName();
    string getClientesFileName();
    vector <Cliente> getListaNegra();
    void getListaNegra(vector <Cliente> l);
    tabHFuncionario getTab();
    /* READ FILES */
    ...
    ///\brief Lê o ficheiro funcionariosantigos.txt, que contém a informacao ordenada de um conjunto de funcionários (Classe Funcionário), que já foram despedidos, sendo adicionados à tabela de dispersao tabHFuncionario.
