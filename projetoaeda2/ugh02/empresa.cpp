...
int Funcionario::getDisponibilidade() const{ int a; return a; };
void Funcionario::setDisponibilidade(int disponibilidade){return;};
int Funcionario::getManutencoes() const{ int a; return a; };
void Funcionario::setManutencoes(int manutencoes){return;};
bool Funcionario::operator>(Funcionario* t) {return true;}
/*
HEAP_TECNICO Funcionario:: getTecnicoFila() const{HEAP_TECNICO a; return a;};
void Funcionario:: addTecnicoToFila(Funcionario *r){ return;};
void Funcionario:: removeTecnicoFromFila(string nif){return;};
*/
//HASH
int funcionarioHash::operator() (const Funcionario* f) const{
    int v = 0;
    for ( unsigned int i=0; i< f->getNif().size(); i++ )
        v = 37*v + f->getNif()[i];
    return v;
}
bool funcionarioEq::operator() ( const Funcionario* f1, const Funcionario* f2) const{
    return f1->getNif()==f2->getNif();
}
...
int Tecnico::getDisponibilidade() const {
    return disponibilidade;
}
void Tecnico::setDisponibilidade(int disponibilidade) {
    Tecnico::disponibilidade = disponibilidade;
}
int Tecnico::getManutencoes() const {
    return manutencoes;
}
void Tecnico::setManutencoes(int manutencoes) {
    Tecnico::manutencoes = manutencoes;
}

bool Tecnico::operator>(Funcionario* t) {
    if(disponibilidade == t->getDisponibilidade()){
        return manutencoes > t->getManutencoes();
    }
    return disponibilidade > t->getDisponibilidade();
}
/*
HEAP_TECNICO Tecnico::getTecnicoFila() const{
    return tecnicos_fila;
}
void Tecnico::addTecnicoToFila(Funcionario *r){
    tecnicos_fila.push(r);
}
void Tecnico::removeTecnicoFromFila(string nif){
    HEAP_TECNICO aux;
    bool existe = false;
...
void Base::funcionarios_readfile(){
    ...
    while(getline(infile,line)) {
        if (line == ":::::") {
            ...
            else if(funcionario_info[0]=="Tecnico") {
                Tecnico *t = new Tecnico;
                t->setTipoDeFuncionario(funcionario_info[0]);
                t->setNome(funcionario_info[1]);
                t->setNif(funcionario_info[2]);
                t->setData(Data(funcionario_info[3]));
                t->setSalario(stoi(funcionario_info[4]));
                t->setDisponibilidade(stoi(funcionario_info[5]));
                t->setManutencoes(stoi(funcionario_info[6]));
...
void Base::funcionariosAntigos_readfile(){
    Data d;
    Veiculo v;
    vector<string> funcionario_info;
    ifstream infile;
    string line,veiculo;
    string temp="";
    infile.open("funcionariosAntigos.txt");
    while(getline(infile,line)) {
        if (line == ":::::") {
            if(funcionario_info[0]=="Entregador") {
                Entregador *e = new Entregador();
                ...
                e->setVeiculoAlocado(Veiculo(funcionario_info[5]));
                e->setDisponivel(stoi(funcionario_info[6]));
                funcionarios_tabH.insert(e);
                getFuncionariosTodos().push_back(e);
                funcionario_info.clear();
            }
            else if(funcionario_info[0]=="Administrador") {
                Administrador *a= new Administrador;
                ...
                a->setSalario(stoi(funcionario_info[4]));
                a->setFuncao(funcionario_info[5]);
                funcionarios_tabH.insert(a);
                getFuncionariosTodos().push_back(a);
                funcionario_info.clear();
            }else if(funcionario_info[0]=="Tecnico") {
                Tecnico *t = new Tecnico;
                t->setTipoDeFuncionario(funcionario_info[0]);
...
void Base::addRestaurante(){
    ...
    if(con!="0" && con!="1") {
        ...
        if(culi!="1" && culi!="0") {
            ...
            restaurantes.push_back(r);
            cout << "O restaurante "<<r.getNome()<<" foi adicionado com sucesso \ne ja se encontra disponivel para receber emcomendas! "<<endl;
...
void Base::addAdmin(){
    ...
    for(int i=0;i<getFuncionariosTodos().size(); i++){
        if(getFuncionariosTodos()[i]->getTipoDeFuncionario()=="Administrador"){
            bool atual=false;
            for(int j=0; j<getFuncionarios().size(); j++){
                if(getFuncionariosTodos()[i]->getNif()==getFuncionarios()[j]->getNif()){
                    atual=true;
                    break;
                }
            }
            if(!atual){
                cout<<"\t\t\t"<<"   " <<getFuncionariosTodos()[i]->getNome()<<" : \t"<<getFuncionariosTodos()[i]->getFuncao()<<endl;
            }
        }
    }
    cout<<endl;
    cout<<"Politica interna: contratacao de funcionarios ja conhecidos."<<endl;
    for(int i=0;i<getFuncionariosTodos().size(); i++) {
        if(getFuncionariosTodos()[i]->getTipoDeFuncionario()=="Administrador"){
            bool atual = false;
            for (int j = 0; j < getFuncionarios().size(); j++) {
            ...
            if (!atual) {
                cout <<" {" <<i<<"} - "<< "Adicionar " << getFuncionariosTodos()[i]->getNome()  << endl;
            }
        }
    }
    cout<<" {0} - Adicionar novo funcionario" <<endl;
    int tab;
    cin>>tab;
    if(tab==0) {
        cin.ignore();
        ...
        file << a.getNome() << endl;
        file << a.getNif() << endl;
        file << to_string(a.getData().getDia()) + "/" + to_string(a.getData().getMes()) + "/" +
                to_string(a.getData().getAno()) << endl;
        file << to_string(a.getSalario()) << endl;
        file << a.getFuncao() << endl;
    }
    else{
        Funcionario *f = getFuncionariosTodos()[tab];
        file.open(funcionarios_filename, ios::app);
        file << ":::::" << endl;
        file << "Administrador" << endl;
        file << f->getNome() << endl;
        file << f->getNif() << endl;
        file << to_string(f->getData().getDia()) + "/" + to_string(f->getData().getMes()) + "/" +
                to_string(f->getData().getAno()) << endl;
        file << to_string(f->getSalario()) << endl;
        file << f->getFuncao() << endl;
        cout<<"O funcionario "<<getFuncionariosTodos()[tab]->getNome()<<" foi adicionado de novo com sucesso!"<<endl;
    }
}
void Base::addEntreg() {
    Entregador a;
    ...
    Veiculo v;
    ofstream file;
    string nome,nif,dia,mes,ano,salario,marca, tipo, diav, data,mesv, anov,datav, veiculo, disp, matricula;
    int b, km;
    cin.ignore();

    cout<<"\tListagem de funcionarios antigos:"<<endl;
    for(int i=0;i<getFuncionariosTodos().size(); i++){
        if(getFuncionariosTodos()[i]->getTipoDeFuncionario()=="Entregador"){
            bool atual=false;
            for(int j=0; j<getFuncionarios().size(); j++){
                if(getFuncionariosTodos()[i]->getNif()==getFuncionarios()[j]->getNif()){
                    atual=true;
            ...
            if(!atual){
                cout<<"\t" <<getFuncionariosTodos()[i]->getNome()<<" - "<<getFuncionariosTodos()[i]->getNif() <<endl;
            }
    ...
    if(tab==0) {
        ...
        cout << "Matricula: (XX-00-XX)" << endl;
        cin>>matricula;
        v.setMatricula(matricula);
        cout << "Nr de kilometros" << endl;
        cin>>km;
        v.setKm(km);
        v.setNumEntregas(0);
        ...
        file << to_string(a.getData().getDia()) + "/" + to_string(a.getData().getMes()) + "/" +
                to_string(a.getData().getAno()) << endl;
        file << to_string(a.getSalario()) << endl;
        if(a.getVeiculoAlocado().isPrecisaArranjo()) {
            file << a.getVeiculoAlocado().getMarca() + ", " + a.getVeiculoAlocado().getTipo() + ", " +
                     a.getVeiculoAlocado().getDataDeAquisicao() + ", " + a.getVeiculoAlocado().getMatricula() + ", " +
                     to_string(a.getVeiculoAlocado().getKm()) + ", " + to_string(a.getVeiculoAlocado().getNumEntregas()) +
                     ", 1" << endl;
        }
        else{
            file << a.getVeiculoAlocado().getMarca() + ", " + a.getVeiculoAlocado().getTipo() + ", " +
                     a.getVeiculoAlocado().getDataDeAquisicao() + ", " +  a.getVeiculoAlocado().getMatricula() +  ", " +
                     to_string(a.getVeiculoAlocado().getKm()) + ", " + to_string(a.getVeiculoAlocado().getNumEntregas()) + ", 0" << endl;
        }
        ...
        file << to_string(f->getData().getDia()) + "/" + to_string(f->getData().getMes()) + "/" +
                to_string(f->getData().getAno()) << endl;
        file << to_string(f->getSalario()) << endl;
        if(f->getVeiculoAlocado().isPrecisaArranjo()) {
            file << f->getVeiculoAlocado().getMarca() + ", " + f->getVeiculoAlocado().getTipo() + ", " +
                    f->getVeiculoAlocado().getDataDeAquisicao() + ", " + f->getVeiculoAlocado().getMatricula() + ", " +
                    to_string(f->getVeiculoAlocado().getKm()) + ", " + to_string(f->getVeiculoAlocado().getNumEntregas()) +
                    ", 1" << endl;
        }
        else{
            file << f->getVeiculoAlocado().getMarca() + ", " + f->getVeiculoAlocado().getTipo() + ", " +
                    f->getVeiculoAlocado().getDataDeAquisicao() + ", " +  f->getVeiculoAlocado().getMatricula() +  ", " +
                    to_string(f->getVeiculoAlocado().getKm()) + ", " + to_string(f->getVeiculoAlocado().getNumEntregas()) + ", 0" << endl;
        }
        file << to_string(f->isDisponivel()) << endl;
        cout<<"O funcionario "<<getFuncionariosTodos()[tab]->getNome()<<" foi adicionado de novo com sucesso!"<<endl;
    }
}
void Base::addTecnico(){
    Tecnico a;
    Data d;
    ofstream file;
    string nome, nif, dia, mes, ano, salario, disponibilidade, manutencoes, data;
    int b;
    cin.ignore();
    cout<<"\tListagem de funcionarios antigos:"<<endl;
    for(int i=0;i<getFuncionariosTodos().size(); i++){
        if(getFuncionariosTodos()[i]->getTipoDeFuncionario()=="Tecnico"){
            bool atual=false;
            for(int j=0; j<getFuncionarios().size(); j++){
                if(getFuncionariosTodos()[i]->getNif()==getFuncionarios()[j]->getNif()){
                    atual=true;
            ...
            if(!atual){
                cout<<"\t\t\t\t\t\t"<<getFuncionariosTodos()[i]->getNome()<<endl;
            }
        }
    }
    cout<<endl;
    ...
    if(tab==0) {
        ...
        a.setManutencoes(0);
        funcionarios.push_back(&a);
        cout << "O tecnico " << a.getNome() << " foi adicionado com sucesso! " << endl;
        file.open(funcionarios_filename, ios::app);
        file << ":::::" << endl;
        file << "Tecnico" << endl;
        file << a.getNome() << endl;
        ...
        file << to_string(a.getData().getDia()) + "/" + to_string(a.getData().getMes()) + "/" +
                to_string(a.getData().getAno()) << endl;
        file << to_string(a.getSalario()) << endl;
        file << a.getDisponibilidade() << endl;
        file << a.getManutencoes() << endl;
    }
    else{
        Funcionario *f = getFuncionariosTodos()[tab];
        file.open(funcionarios_filename, ios::app);
        ...
        file << to_string(f->getData().getDia()) + "/" + to_string(f->getData().getMes()) + "/" +
                to_string(f->getData().getAno()) << endl;
        file << to_string(f->getSalario()) << endl;
        file << f->getDisponibilidade() << endl;
        file << f->getManutencoes() << endl;
        cout<<"O funcionario "<<getFuncionariosTodos()[tab]->getNome()<<" foi adicionado de novo com sucesso!"<<endl;
...
void Base::removeAdmin(Funcionario f) {
    ofstream file1;
    file1.open("funcionariosantigos.txt", ios::app);
    file1 << ":::::" << endl;
    file1 << "Administrador" << endl;
    file1 << f.getNome() << endl;
    file1 << f.getNif() << endl;
    file1 << to_string(f.getData().getDia()) + "/" + to_string(f.getData().getMes()) + "/" +
             to_string(f.getData().getAno()) << endl;
    file1 << to_string(f.getSalario()) << endl;
    file1 << f.getFuncao() << endl;
    file1.close();
    bool a=false;
    ofstream file;
    for(int i=0; i<funcionarios.size();i++){
    ...
    if(a==true) {
        ...
        for (int i = 0; i < funcionarios.size() - 1; i++) {
            if (funcionarios[i]->getTipoDeFuncionario() == "Administrador") {
                file << funcionarios[i]->getTipoDeFuncionario() << endl;
                ...
                file << to_string(funcionarios[i]->getData().getDia()) + "/" +
                        ...
                        to_string(funcionarios[i]->getData().getAno()) << endl;
                file << to_string(funcionarios[i]->getSalario()) << endl;
                if(funcionarios[i]->getVeiculoAlocado().isPrecisaArranjo()==true) {
                    file << funcionarios[i]->getVeiculoAlocado().getMarca() + ", " +
                            funcionarios[i]->getVeiculoAlocado().getTipo() + ", " +
                            funcionarios[i]->getVeiculoAlocado().getDataDeAquisicao() + ", " +
                            funcionarios[i]->getVeiculoAlocado().getMatricula() + ", " +
                            to_string(funcionarios[i]->getVeiculoAlocado().getKm()) + ", " +
                            to_string(funcionarios[i]->getVeiculoAlocado().getNumEntregas()) + ", " +
                            "1" << endl;
                }else{
                    file << funcionarios[i]->getVeiculoAlocado().getMarca() + ", " +
                            funcionarios[i]->getVeiculoAlocado().getTipo() + ", " +
            ...
            else if (funcionarios[i]->getTipoDeFuncionario() == "Tecnico") {
                file << funcionarios[i]->getTipoDeFuncionario() << endl;
                file << funcionarios[i]->getNome() << endl;
                file << funcionarios[i]->getNif() << endl;
                file << to_string(funcionarios[i]->getData().getDia()) + "/" +
                        ...
                        to_string(funcionarios[i]->getData().getAno()) << endl;
                file << to_string(funcionarios[i]->getSalario()) << endl;
                file << funcionarios[i]->getDisponibilidade() << endl;
                file << funcionarios[i]->getManutencoes() << endl;
                file << ":::::" << endl;
            }
        }
        if (funcionarios[funcionarios.size() - 1]->getTipoDeFuncionario() == "Administrador") {
            file << funcionarios[funcionarios.size() - 1]->getTipoDeFuncionario() << endl;
            ...
            file << to_string(funcionarios[funcionarios.size() - 1]->getData().getDia()) + "/" +
                    ...
                    to_string(funcionarios[funcionarios.size() - 1]->getData().getAno()) << endl;
            file << to_string(funcionarios[funcionarios.size() - 1]->getSalario()) << endl;
            if(funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().isPrecisaArranjo()==true) {
                file << funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().getMarca() + ", " +
                        ...
                        funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().getDataDeAquisicao() + ", " +
                        funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().getMatricula() + ", " +
                        to_string(funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().getKm()) + ", " +
                        to_string(funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().getNumEntregas()) + ", " +
                        "1" << endl;
            }else{
                file << funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().getMarca() + ", " +
                        funcionarios[funcionarios.size() - 1]->getVeiculoAlocado().getTipo() + ", " +
        ...
        else if (funcionarios[funcionarios.size() - 1]->getTipoDeFuncionario() == "Tecnico") {
            file << funcionarios[funcionarios.size() - 1]->getTipoDeFuncionario() << endl;
            file << funcionarios[funcionarios.size() - 1]->getNome() << endl;
            file << funcionarios[funcionarios.size() - 1]->getNif() << endl;
            file << to_string(funcionarios[funcionarios.size() - 1]->getData().getDia()) + "/" +
                    ...
                    to_string(funcionarios[funcionarios.size() - 1]->getData().getAno()) << endl;
            file << to_string(funcionarios[funcionarios.size() - 1]->getSalario()) << endl;
            file << funcionarios[funcionarios.size() - 1]->getDisponibilidade() << endl;
            file << funcionarios[funcionarios.size() - 1]->getManutencoes() << endl;
        }
        file.close();
...
void Base::removeEntreg(Funcionario* f) {
    ...
    file1 << to_string(f->getSalario()) << endl;
    if(f->getVeiculoAlocado().isPrecisaArranjo()) {
        file1 << f->getVeiculoAlocado().getMarca() + ", " + f->getVeiculoAlocado().getTipo() + ", " +
                 f->getVeiculoAlocado().getDataDeAquisicao() + ", " + f->getVeiculoAlocado().getMatricula() + ", " +
                 to_string(f->getVeiculoAlocado().getKm()) + ", " + to_string(f->getVeiculoAlocado().getNumEntregas()) +
                 ", 1" << endl;
    }
    ...
    file1 << to_string(f->isDisponivel()) << endl;
    file1.close();
    bool a=false;
    ofstream file;
    for(int i=0; i<funcionarios.size();i++){
    ...
    if(a==true) {
        ...
        cout<<"O nif que selecionou nao pertence a nenhum entregador!"<<endl;
    }
}
void Base::removeTecnico(Funcionario f) {
    ofstream file1;
    file1.open("funcionariosantigos.txt", ios::app);
    file1 << ":::::" << endl;
    file1 << "Tecnico" << endl;
    ...
    file1 << to_string(f.getData().getDia()) + "/" + to_string(f.getData().getMes()) + "/" +
             to_string(f.getData().getAno()) << endl;
    file1 << to_string(f.getSalario()) << endl;
    file1 << to_string(f.getDisponibilidade()) << endl;
    file1 << to_string(f.getManutencoes()) << endl;
    file1.close();
    bool a=false;
    ofstream file;
    for(int i=0; i<funcionarios.size();i++){
        if(funcionarios[i]->getNif()==f.getNif()){
            if(funcionarios[i]->getTipoDeFuncionario()=="Tecnico"){
                funcionarios.erase(funcionarios.begin()+i);
                removeTecnicoFromFila(funcionarios[i]->getNif());
                a=true;
            }else{
                a=false;
            }
    ...
    if(a==true) {
        ...
        cout<<"O nif que selecionou nao pertence a nenhum funcionario!"<<endl;
    }
}
...
vector<Funcionario*> Base::getFuncionariosTodos(){
    vector<Funcionario*> ret;
    for (auto f : funcionarios_tabH) {
        ret.push_back(f);
    }
    return ret;
}
void Base::setFuncionariosTodos(const vector<Funcionario* > &funcionariosv){
    funcionarios_tabH.clear();
    Funcionario * f;
    for(int i=0; i<funcionariosv.size(); i++){
        f=funcionariosv[i];
        funcionarios_tabH.insert(f);
    }
}
...
/*
HEAP_TECNICO Empresa::getTecnicoFila() const{
    return tecnicos_fila;
}
void Empresa::addTecnicoToFila(Funcionario *r){
    tecnicos_fila.push(r);
}
void Empresa::removeTecnicoFromFila(string nif){
    HEAP_TECNICO aux;
    bool existe = false;
    if(tecnicos_fila.empty()){
        existe=false;
